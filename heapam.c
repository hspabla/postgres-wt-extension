					...
					ScanKey key)
{
	...
	for (;;)
	{
		while (linesleft > 0)
		{
			...
			lpp = PageGetItemId(dp, lineoff);
			Assert(ItemIdIsNormal(lpp));
         Item *item = palloc( ItemIdGetLength(lpp) );

         if( strncmp( RelationGetRelationName( scan->rs_rd ), "wt_", 3 ) == 0 )
         {
            char* key_str = palloc( sizeof(OffsetNumber) );
            memcpy( key_str, &lineoff, sizeof(OffsetNumber) );

            tuple->t_len = wtread( scan->rs_rd, page, key_str,
                                   sizeof(OffsetNumber), item );
            tuple->t_data = item;
            pfree( key_str );
         }
         else
