					 ...
					 HeapTuple tuple)
{
   ...
   // if its wiredtiger rel, we put this tup into wt storage
   if( strncmp( RelationGetRelationName( relation ), "wt_", 3 ) == 0 ) {
      char* key_str = palloc( sizeof(OffsetNumber) );
      memcpy( key_str, &(tuple->t_self.ip_posid), sizeof(OffsetNumber) );

      if ( wtexists( relation, BufferGetBlockNumber(buffer) ) ) {
         wtwrite( relation, BufferGetBlockNumber(buffer),
                            key_str, sizeof(OffsetNumber),
                            tuple->t_data,
                            tuple->t_len );

      } else {
         wtcreate ( relation, BufferGetBlockNumber(buffer) );
         wtwrite( relation, BufferGetBlockNumber(buffer),
                  key_str, sizeof(OffsetNumber),
                  (Item) tuple->t_data, tuple->t_len );
      }
      pfree( key_str );
   }
